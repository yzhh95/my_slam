...
vector<cv::Point2f> FeatureTracker::ptsVelocity(vector<int> &ids, vector<cv::Point2f> &pts, map<int,cv::Point2f> &id_pts)
{
    ...
    if (!id_pts.empty())
    {
        ...
        for (size_t i = 0; i < cur_id_pts.size(); i++)
        {
            ...
            if( id_pts.find(ids[i]) != id_pts.end()) 
            {
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double t, cv::Mat &Imgleft, cv::Mat &Imgright)
{
    ...
    if (prev_pts.size() > 0)
    {
        vector<uchar> status;
        vector<float> err;
        cv::calcOpticalFlowPyrLK(prev_img, Imgleft, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
        ROS_DEBUG("prev_pts.size: %d", prev_pts.size());
        ROS_DEBUG("status.size: %d", status.size());
    ...
    {
        n_pts.clear();
        cv::goodFeaturesToTrack(Imgleft, n_pts, MAX_CNT - cur_pts.size(), 0.01, MIN_DIST, mask);
        for (auto &p : n_pts)
        {
            cur_pts.push_back(p);
            ids.push_back(n_id++);
